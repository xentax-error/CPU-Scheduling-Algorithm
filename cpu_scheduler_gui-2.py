...
# =============================================================================
# Two modes controlled by a toggle switch:
#   Simplified (OFF): Shows only the boundary times at each block edge.
#   Detailed   (ON):  Shows every time unit 0..n on a full number line,
#                     with divider lines inside each block at each unit.

class GanttChart(ctk.CTkFrame):

    BLOCK_H = 40   # height of process blocks in pixels
    PAD_X   = 16   # horizontal padding from canvas edge to first block
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.toggle_switch = ctk.CTkSwitch(
            ...
            fg_color=C_BORDER,
            command=self.redraw)
        self.toggle_switch.pack(side="left", padx=6)

        # This label updates when the switch is toggled
        self.toggle_label = ctk.CTkLabel(toggle_row,
                                          text="OFF — shows block boundary times only",
        ...
        self.canvas.pack(fill="both", expand=True, padx=4, pady=(0, 6))
    ...
    def redraw(self):
        ...
        if self.detailed_var.get():
            self.toggle_label.configure(
                text="ON  — shows every time unit with full number line",
                text_color=C_BLUE)
        else:
            self.toggle_label.configure(
                text="OFF — shows block boundary times only",
    ...
    def _render(self, timeline, processes, detailed):
        ...
        # Merge consecutive same-pid segments so the chart stays clean
        merged = []
        ...
        # Detailed mode: at least 28px so numbers like "10", "11" don't overlap.
        # Simplified: wider is fine, just needs to look good.
        if detailed:
            px = max(28, min(56, 700 // max(total_time, 1)))
        else:
            px = max(30, min(72, 640 // max(total_time, 1)))

        # Layout constants
        BLOCK_TOP = 6
        BLOCK_BOT = BLOCK_TOP + self.BLOCK_H
        PAD_X     = self.PAD_X
        TICK_TOP  = BLOCK_BOT + 1    # where tick lines start
        TICK_BOT  = BLOCK_BOT + 6    # where tick lines end
        LABEL_Y   = TICK_BOT + 1     # where number labels sit

        # Canvas dimensions
        canvas_w = PAD_X * 2 + total_time * px + 10
        canvas_h = LABEL_Y + 14      # enough room for the numbers below ticks
        ...
        for start, end, pid in merged:
            ...
            # In detailed mode: draw a thin dashed divider at every unit inside
            if detailed:
                for t in range(start + 1, end):
                    tx = PAD_X + t * px
                    self.canvas.create_line(
                        tx, BLOCK_TOP + 2, tx, BLOCK_BOT - 2,
                        fill="#00000050", width=1, dash=(2, 3))

            # Process name label centered in block
            font_size = 8 if (x1 - x0) < 26 else 10
        ...
        # --- Draw axis -------------------------------------------------------
        if detailed:
            # Full number line: tick + number for every single time unit 0..n
            for t in range(total_time + 1):
                tx = PAD_X + t * px
                # Tick mark below block
                self.canvas.create_line(
                    tx, TICK_TOP, tx, TICK_BOT,
                    fill=C_DIM, width=1)
                # Number centered on the tick
                self.canvas.create_text(
                    tx, LABEL_Y,
                    text=str(t), anchor="n",
                    fill=C_TEXT, font=("Segoe UI", 8))
        else:
            # Simplified: only the boundary times at block edges
            shown = set()
...
class ProcessRow(ctk.CTkFrame):
    ...
    def __init__(self, parent, number, show_priority, dot_color):
        ...
        self.arr_box = ctk.CTkEntry(self, width=72,
                                     ...
                                     fg_color=C_PANEL, border_color=C_BORDER,
                                     text_color=C_DIM,          # dim = placeholder state
                                     placeholder_text="0")
    ...
    def _setup_placeholder(self, entry, default_val):
        ...
        def on_focus_in(event):
            if getattr(entry, "_is_placeholder", False):
                entry.delete(0, "end")
                entry.configure(text_color=C_TEXT)
        ...
        def on_focus_out(event):
            if entry.get().strip() == "":
                entry.delete(0, "end")
                entry.insert(0, default_val)
                entry.configure(text_color=C_DIM)
                entry._is_placeholder = True
    ...
    def clear_inputs(self):
        ...
        self.arr_box.configure(text_color=C_DIM)
        ...
        self.pri_box.configure(text_color=C_DIM)
...
class App(ctk.CTk):
    ...
    def build_window(self):
        ...
        left = ctk.CTkScrollableFrame(body, fg_color=C_BG, width=455,
    ...
    def build_left_side(self, parent):
        ...
        ctk.CTkOptionMenu(
            ...
            dropdown_hover_color=C_BLUE, corner_radius=8, width=420,
        ...
        count_row.pack(padx=12, pady=(0, 10), anchor="w")
        ...
        self.primode_menu = ctk.CTkOptionMenu(
            ...
            dropdown_fg_color=C_PANEL2, dropdown_text_color=C_TEXT,
            dropdown_hover_color=C_BLUE, corner_radius=6, width=420,
        )
        ...
        off_frame = ctk.CTkFrame(self.preempt_panel, fg_color=C_PANEL2, corner_radius=8)
        off_frame.pack(fill="x", padx=12, pady=(0, 4))
        ctk.CTkLabel(off_frame, text="OFF",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color=C_GREEN, width=36, anchor="center"
        ...
        on_frame = ctk.CTkFrame(self.preempt_panel, fg_color=C_PANEL2, corner_radius=8)
        ...
        ctk.CTkButton(clear_row, text="Clear All",
    ...
    def build_right_side(self, parent):
        ...
        gantt_scroll = ctk.CTkScrollableFrame(
            ...
            orientation="horizontal", height=120, corner_radius=10,
            scrollbar_button_color=C_BORDER,
            scrollbar_button_hover_color=C_BLUE)
        gantt_scroll.pack(fill="x", pady=(0, 12))

        self.gantt = GanttChart(gantt_scroll, height=106)
    ...
    def on_preempt_toggled(self, *_):
        if self.preempt_var.get():
            ...
            self.preempt_status.configure(
                text="OFF  (Non-Preemptive)", text_color=C_GREEN)
    ...
    def run_simulation(self):
        ...
        try:
            ...
            elif key == "Priority+RR": result, tl = run_priority_rr(processes, mode, quantum)
    ...
    def show_results(self, processes, timeline, algo):
        ...
        if algo["key"] == "Priority":
            mode_text = (" [Preemptive]" if self.preempt_var.get()
                         else " [Non-Preemptive]")
        self.result_desc.configure(
            text=f"{algo['name']}{mode_text}  ·  {algo['type']}\n{algo['description']}")
